...
function build_mainland_re_blocks(avail_pv::Vector{Float64},
    ...
    # §5: 블록별 설비용량 (installed_mw = w_block × ρ × installed_total)
    pv_low_inst  = w_pv[1] * rho_pv * installed_pv
    pv_mid_inst  = w_pv[2] * rho_pv * installed_pv
    pv_high_inst = w_pv[3] * rho_pv * installed_pv
    w_low_inst   = w_w[1]  * rho_w  * installed_w
    w_mid_inst   = w_w[2]  * rho_w  * installed_w
    w_high_inst  = w_w[3]  * rho_w  * installed_w

    # 시나리오별 가격 부여 (M-Rule: bid_max = 0)
    ...
    blocks = RenewableBidBlock[
        RenewableBidBlock("PV_low",  "solar", pv_low_avail,  make_bid(:low),  pv_low_inst),
        RenewableBidBlock("PV_mid",  "solar", pv_mid_avail,  make_bid(:mid),  pv_mid_inst),
        RenewableBidBlock("PV_high", "solar", pv_high_avail, make_bid(:high), pv_high_inst),
        RenewableBidBlock("W_low",   "wind",  w_low_avail,   make_bid(:low),  w_low_inst),
        RenewableBidBlock("W_mid",   "wind",  w_mid_avail,   make_bid(:mid),  w_mid_inst),
        RenewableBidBlock("W_high",  "wind",  w_high_avail,  make_bid(:high), w_high_inst),
    ]
...
function solve_post_ed(input::PostEDInput;
    ...
    # §3: bidding_active=false 인 경우 K=0 (입찰 블록 변수 생략)
    K = bidding_active ? length(input.re_blocks) : 0
    ...
    for k in 1:K, t in 1:T
        ...
        # §5: 제주 규정 — Pmin = min(α × installed_mw, A_kt)
        # installed_mw == 0 이면 fallback (α × avail).
        if re_pmin_frac > 0.0 && avail_kt > 1e-3
            inst = input.re_blocks[k].installed_mw
            pmin_kt = inst > 0.0 ? min(re_pmin_frac * inst, avail_kt) :
                                    re_pmin_frac * avail_kt
            set_lower_bound(r[k, t], pmin_kt)
        end
    end
    ...
    # (R1) 목적함수 — §3: re_net 에 ε > 0 을 부여하여 degeneracy 해소
    if use_piecewise
        @objective(model, Min,
            ...
            sum(input.re_blocks[k].bid[t] * r[k, t]
                for k in 1:K, t in 1:T) +
            epsilon_nonbid * sum(re_net[t] for t in 1:T)
        )
    else
        @objective(model, Min,
