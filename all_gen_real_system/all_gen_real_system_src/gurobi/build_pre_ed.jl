...
- true : RE 를 must-take 상수로 주입. RE > demand 인 경우만 demand 로 clip.
  Calibration 단계에서 사용 — adder 가 curtailment dual pollution 을
  흡수하지 않도록 하기 위함 (§9).
"""
function solve_pre_ed(input::PreEDInput;
                      pw_costs::Vector{PiecewiseCost}=PiecewiseCost[],
                      curtailment_free::Bool=false)
    T = input.base.T
    ...
    if curtailment_free
        # §9: RE must-take. demand 초과 시 demand 로 clip.
        for t in 1:T
            if effective_re[t] > input.base.demand[t]
                re_curtailed[t] = effective_re[t] - input.base.demand[t]
                effective_re[t] = input.base.demand[t]
            end
        end
        if sum(re_curtailed) > 1e-3
            @warn "Pre ED [curtailment_free=true]: RE > demand 시간대 $(count(c->c>1e-3, re_curtailed))건 demand 로 clip"
        end
    else
        # 현행 시장: must-run pmin 보전을 위한 사전 cap
        for t in 1:T
